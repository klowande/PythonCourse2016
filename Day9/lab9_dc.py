...
for town in session.query(Town).filter(Town.population>100000):
  department = session.query(Department).filter(Department.id==town.dept_id)
  print department[0].deptname, town.name, town.population

# 2. aDisplay the list of all the one-way connections between two cities for which the population of one of the 2 cities is lower than 80000 inhabitants. 

for connection in session.query(Town, Distance).filter(Town.population < 80000).filter(or_(Distance.towndepart == Town.name, Distance.townarrive == Town.name)):
    print connection.Distance

#using SQL commands

stmt = '''
SELECT id, towndepart, townarrive FROM distances
WHERE EXISTS (SELECT * FROM towns WHERE population<80000
AND (towndepart = name OR townarrive = name))
'''
...
for department in session.query(Department):
    ...
    for town in session.query(Town).filter(Town.dept_id == department.id):
        pop+=town.population
    print department, pop

for department in session.query(Department).order_by(Department.deptname):
    if len(department.towns)>0: print department.deptname, reduce(lambda x,y: x+y, [town.population for town in department.towns])
    
#using SQL commands

stmt = '''SELECT dept_id, SUM(population) FROM towns GROUP BY dept_id;'''
result=engine.execute(stmt)
for row in result: print row
# hint: use func.sum
